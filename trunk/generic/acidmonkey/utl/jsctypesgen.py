...
# improved by Petko D. (pdp) Petkov
# GNUCITIZEN
...
class JSCTypesGen:
	...
	def writeFunction(self, declaration, out):
		...
		if declaration.arguments:
			args_types = ', ' + ', '.join(self.ctypesNameForType(arg.type) for arg in declaration.arguments)
		
		out.write('\tthis.%s = lib.declare(\'%s\', ctypes.default_abi, %s%s);\n' % (name, name, return_type, args_types));
	...
	def writeStruct(self, declaration, out):
		...
		try:
			members = ', '.join('{%s: %s}' % (member.name, self.ctypesNameForType(member.type)) for member in declaration.get_members() if isinstance(member, pygccxml.declarations.variable_t))
	...
	#
	# Resolving pygccxml writters.
	#
	writers = {
		pygccxml.declarations.typedef_t: 'writeTypedef',
	...
	def ctypesNameForType(self, typ):
		...
		if isinstance(typ, pygccxml.declarations.free_function_type_t):
			...
			args_types = [self.ctypesNameForType(arg_type) for arg_type in typ.arguments_types]
			
			return ('new ctypes.FunctionType(ctypes.default_abi, %s, [%s])' % (return_type, ', '.join(args_types)))
...
%(declarations)s}\n\n'''
	...
	def writeFile(self, filename, decls):
		...
		out.write(self.fileTemplate % {'header':mangled, 'upper':mangled.upper(), 'declarations':tempout.getvalue(), 'filename':filename})
		out.close()

def main():
	import sys
	
	gccxml_path = os.getenv('GCCXML_PATH') or '/usr/bin/gccxml'
	gcc_path = os.getenv('GCC_PATH') or '/usr/bin/gcc'
	cflags = sys.argv[1:]
	filename = cflags.pop()
	config = pygccxml.parser.config_t(gccxml_path=gccxml_path, cflags=' '.join(cflags), compiler=gcc_path)
